...
class QwenLoraExpirement(Expirement):
    ...
    def __init__(self, dataset: DatasetDict):
        super().__init__(
            metrics=ExpirementMetrics(
                title="Qwen/Qwen3-1.7B-supervised-lora-fine-tuning",
                description="Супервизорное QLora дообучение модели Qwen/Qwen3-1.7B с последующим инференсом и оценкой на тестовом наборе данных по метрикам BLEU и ROUGE."
            )
        )

        self.modelDir = "./fine_tuned_models/lora/Qwen3-1.7B"
        self.modelName = "Qwen/Qwen3-1.7B-lora"
    ...
    def run(self):
        ...
        base_model = AutoModelForCausalLM.from_pretrained("Qwen/Qwen3-1.7B", cache_dir=CACHE_DIR).to(device)
        model = PeftModel.from_pretrained(base_model, self.modelDir).to(device)
        tokenizer = AutoTokenizer.from_pretrained("Qwen/Qwen3-1.7B", cache_dir=CACHE_DIR)
        model.eval()
