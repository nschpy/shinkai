...
class SmolLMLoraExpirement(Expirement):
    ...
    def __init__(self, dataset: DatasetDict):
        ...
        self.modelDir = "./fine_tuned_models/lora/SmolLM2-1.7B"
        self.modelName = "HuggingFaceTB/SmolLM2-1.7B-lora"
    ...
    def run(self):
        ...
        modelConfig = PeftConfig.from_pretrained(self.modelDir)
        loaded_model = AutoModelForCausalLM.from_pretrained(modelConfig.base_model_name_or_path, cache_dir=CACHE_DIR).to(device)
        model = PeftModel.from_pretrained(loaded_model, self.modelDir).to(device)
        tokenizer = AutoTokenizer.from_pretrained(modelConfig.base_model_name_or_path, cache_dir=CACHE_DIR)
        model.eval()
